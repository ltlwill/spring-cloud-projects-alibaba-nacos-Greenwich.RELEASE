	...
	public GatewayFilter apply(Object config) {
		return (exchange, chain) -> {
			...
			if (StringUtils
					.endsWithIgnoreCase(
							path,
							GatewayAggregationSwaggerResourcesProvider.SWAGGER_DOCS_PATH)) {
				return chain.filter(exchange);
			}
			String basePath = path
					.substring(
							0,
							path.lastIndexOf(GatewayAggregationSwaggerResourcesProvider.SWAGGER_DOCS_PATH));
